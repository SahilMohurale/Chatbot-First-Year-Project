...
def takecommand():
   ...
   # with sr.Microphone() as source:
    ...
    try:
        ...
        query = query.lower()
        canvas2.create_text(490, 120, anchor=NE, justify=RIGHT, text=query, font=('fixedsys', -30), fill="white",
                           width=350)
        global img3
...
def web_scraping(qs):
    ...
    if 'play' in qs:
        song = qs.replace('play', '')
        speak('playing ' + song)
        pywhatkit.playonyt(song)
    elif 'time' in qs:
        time = datetime.datetime.now().strftime('%I:%M %p')
        speak('Current time is ' + time)
    elif 'who the heck is' in qs:
        person = qs.replace('who the heck is', '')
        ...
        speak('Sahil Mohurale')
    elif 'are you single' in qs:
...
def wishme():
    ...
    if 0 <= hour < 12:
        text = "Good Morning sir. I am Sahil. How can I Serve you?"
    elif 12 <= hour < 18:
        text = "Good Afternoon sir. I am Sahil. How can I Serve you?"
    else:
        text = "Good Evening sir. I am Sahil. How can I Serve you?"
